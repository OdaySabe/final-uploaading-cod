...
Start the server:
=======================================================*/
app.get("/firstEX", function (req, res) {
...
app.get("/secondEX", function (req, res) {
...
app.get("/ThirdEx", function (req, res) {
...
app.get("/fourthEX", function (req, res) {
...
app.get("/fifthEX", function (req, res) {
  //Find all the people who are tall (>180) OR rich (>30000)
  Person.find(
...
app
  .get("/fifthEX", function (req, res) {
    Person.find().$and([
      ({
        $or: [{ hair: "grey" }, { eyes: "gray" }],
      },
      { weight: { $ls: 70 } }),
    ]);
  })
...
app.get("/sixthEX", function (req, res) {
  //Find people who have at least 1 kid with grey hair
  Person.find({
    kids: { $elemMatch: { hair: "gray" } },
  }).exec(function (error, people) {
    res.send(person);
  });
});

app.get("/seventhEX", function (req, res) {
